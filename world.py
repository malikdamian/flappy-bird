...
class World:
    ...
    def _add_pipe(self):
        ...
        top_pipe_height, bottom_pipe_height = (
            pipe_pair_size[0] * pipe_size,
            pipe_pair_size[1] * pipe_size,
        )
        pipe_top = Pipe(
    ...
    def _handle_collisions(self):
        ...
        group = pygame.sprite.groupcollide(self.player, self.pipes, False, False)
        if group or bird.rect.bottom >= HEIGHT or bird.rect.top <= 0:
            self.playing = False
            self.game_over = True
        else:
            bird = self.player.sprite
